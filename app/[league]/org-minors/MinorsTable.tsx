...
export default function MinorsTable({ rows, teamCounts, roleHealth }: { rows: MinorsPlayerRow[]; teamCounts: TeamPositionCounts[]; roleHealth: RoleHealthRow[] }) {
  // Multi-league routing (2026-09-10) -- this table is rendered from
  // app/[league]/org-minors/page.tsx, so the current league slug is always
  // in the URL; useParams() is the idiomatic way for a client component to
  // read it without needing a new prop threaded through the page.
                    ...
                    <tr key={r.player_id}>
                      <td style={{ padding: "3px 8px", borderBottom: "1px solid var(--color-border, #333)", whiteSpace: "nowrap" }}>
                        {/* Name links to our own player detail page
                            ...
                            "↗" link right after. */}
                        <Link href={`/${league}/players/${r.player_id}`} style={{ color: "inherit" }}>
