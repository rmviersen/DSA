    ...
    <div className="prospects-report-page">
      ...
      <div style={{ border: "1px solid var(--color-border)", borderRadius: 8, padding: "12px 16px", background: "var(--color-surface)", marginBottom: 16, fontSize: "0.875rem", lineHeight: 1.5 }}>
        ...
        <p style={{ margin: "0 0 8px" }}>
          Player grades are generated by our internal evaluation system, which runs entirely independent of the game&apos;s
          own overall and potential ratings. It works from raw tools only — hitting, fielding, baserunning, and pitching —
          and builds every grade from scratch, calibrated against actual outcomes across the league rather than scouting
          intuition alone. Positional value is built in directly — a shortstop or catcher grades out ahead of a first
          baseman or DH carrying the same raw tools, reflecting how hard each position actually is to fill.
        </p>
        <p style={{ margin: 0 }}>
          Grades also account for development timeline, injury history, and makeup traits such as work ethic and
          intelligence, and hitters are evaluated against the league&apos;s real left/right pitching mix rather than a
          generic split. Each prospect is paired with the closest current MLB comparable as a reference point, and the
          model is retrained as new performance data comes in — rankings move with the season, not just each spring.
        </p>
      </div>
